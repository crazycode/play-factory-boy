...
public class ProductTest extends UnitTest {
    ...
    public void testCreateProduct() throws Exception {
        // when use assertDifference, DON'T use lazyDelete(), OR call a create()
        // method at first.
        FactoryBoy.delete(Product.class);
    ...
    public void testBatchCreateProducts() throws Exception {
        ...
        assertDifference(Product.class, 5, new Callback() {
            @Override
            public void run() {
                List<Product> products = FactoryBoy.batchCreate(5,
                                Product.class, new SequenceCallback<Product>() {
                                    @Override
                                    public void sequence(Product target, int seq) {
                                        target.name = "Test Product " + seq;
                                        target.price = BigDecimal.TEN
                                                        .add(new BigDecimal(seq));
                                    }
                                });
    ...
    @Test
    public void testCreateProductSequence() throws Exception {
        FactoryBoy.create(Product.class, "sequence");
        assertEquals(1l, Product.count());
    }

    @Test
    public void testCreateOrderedProduct() throws Exception {
        FactoryBoy.create(Product.class, "random");
        assertEquals(1l, Product.count());
    }
}
